...
from app.services.content.providers import falai
...
class TestPollFailureClassification(unittest.TestCase):
    def test_policy_error_text_raises_content_policy(self):
        response = _response({"status": "FAILED", "error": "content policy violation"})

        with patch.object(falai.requests, "get", return_value=response):
            with self.assertRaises(GenerationError) as ctx:
                falai._poll("k", "https://status", "https://result")

        self.assertEqual(ctx.exception.code, GenerationErrorCode.content_policy)

    def test_non_policy_error_raises_unknown(self):
        response = _response({"status": "FAILED", "error": "upstream timeout"})
    ...
    def test_cancelled_status_is_also_classified(self):
        response = _response({"status": "CANCELLED", "error": "flagged for moderation"})
